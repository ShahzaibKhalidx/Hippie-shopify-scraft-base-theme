(function ($) {
  ...
  Presentation.prototype = $.extend({}, Presentation.prototype, {
    load: function () {
      ...
      this.dom.$presentation.removeClass("d-none");
      ...
      $.each(this.settings.actions, function (i, v) {
        let $button = _.dom.$presentation.find(
          '[data-js-action="' + this + '"]'
        );
        let cookie = theme.Cookie.get("presentation-" + this);

        if (
          cookie === "enabled" &&
          $button.attr("data-invert") !== "true" &&
          !$button.is(":checked")
        ) {
          toggleState(v, true);
      ...
      this.dom.$presentation.on("change", "[data-js-action]", function () {
        ...
        if (state) {
          theme.Cookie.set("presentation-" + action, "enabled", {
            expires: 60 * 60 * 1000,
          });
        } else {
          theme.Cookie.deleteCookie("presentation-" + action);
        }
